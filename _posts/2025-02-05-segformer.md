...
Think of sequence reduction as compressing the "memory" of the image before performing attention. Instead of making every pixel interact with every other pixel, SegFormer groups patches together to form "super-patches", allowing preservation of global context (without computing all pairwise interactions), and efficient feature fusion (while maintaining important information). Imagine we are in a room with $$1000$$ people, and we need to introduce ourselves to everyone -- instead of personally shaking hands with al $$1000$$ individuals ($$N^2$$ complexity), you first group people into clusters of $$10$$ and introduce yourself to one representative per group ($$\frac{N}{10}$$ complexity). We can still get a global underrstand of the crowd but with far fewer interactions. 
