...
<strong>Problem B3:</strong> Let $Q$ be an $n$-by-$n$ real orthogonal matrix, and let $$u \in \mathbb{R}^n$$ be a unit column vector (that is, $$u^Tu = 1$$). Let $$P = I - 2uu^T$$, where $$I$$ is the $$n$$-by-$$n$$ identity matrix. Show that if $$1$$ is not an eigenvalue of $P$, then $1$ is an eigenvalue of $PQ$. 
