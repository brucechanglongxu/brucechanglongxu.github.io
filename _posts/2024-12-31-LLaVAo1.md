...
It is a heuristic search algorithm used in sequence generation tasks (e.g. text generation, translationa, reasoning), and unlike greedy search (which selects the most probable token at each step), beam search explores multiple possible sequences in parallel and selects the best candidates. 
