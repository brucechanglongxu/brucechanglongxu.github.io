...
Let $\mu = (|z_1| + ... + |z_{2019}|)/2019$ be the average of the distances from $z_1, ..., z_{2019}$ to the origin. Determine the largest constant $M$ such that $\mu \ge M$ for all choices of $b_0, b_1, ..., b_{2019}$ that satisfy:

$$\begin{equation} 1 \le b_0 < b_1 < b_2 < ... < b_{2019} \le 2019 \end{equation}$$
...
$$\begin{equation} m(f) = \min_{s \ge 1} (f(s+1,s+1) - f(s+1,s) - f(s,s+1)+f(s,s)) \end{equation}$$
