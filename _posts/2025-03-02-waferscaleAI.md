...
(*) Recall that a **bypass** in a pipelined processor means that an instruction's result is typically written to a register file before the next instruction can use it, introducing data hazards; bypass is a technique where instead of waiting for the register write back stage, the result is directly forwarded from execeution stage to a dependent instruction before it is written to registers. THis reduces stall cycles and improves pipeline efficiency.  
