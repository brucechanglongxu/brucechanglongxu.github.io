...
Now some precuror solutions to the ZeRO methodology are well-published, widely used techniques such as mixed precision training, data parallelism, pipeline parallelism (Gpipe, zerobubble), tensor parallelism (can split row-wise or colum-wise, the gradient is split between devices and we use an `allReduce`) [^9] and sharding. 
