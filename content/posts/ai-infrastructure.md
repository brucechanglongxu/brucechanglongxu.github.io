...
The mental model here is the _roofline_. Imagine a chart with two axes: arithmetic intensity (FLOPs per byte of memory moved) [^7] and achievable performance as a fraction of the device's peak. Your kernels live somewhere on this curve. If they are stuck in the low-intensity, memory-bound region, HBM-bandwidth - not Tensor Cores - is your bottleneck. If they are compute bound, the question becomes whether you are actually saturating those Tensor Cores. And if they are nowhere near the roofline at all, overheads are eating you alive - kernel launch latency, Python-GIL stalls [^1], or CPU-GPU syncs that keep the device idle. 
...
We will dive into ZeRO, which is built on top of data parallelism (as opposed to model parallelism) because even though data parallelism is memory inefficient (model states are stored redundantly across all data-parallel processes), it has better scaling efficiency than model parallelism. Model parallelism [^10] may reduce computational pressure, but it also increases communication overhead - beyond a certain point, this tradeoff is not worth it, because we are using less of the compute provided to us per GPU breaking the model into even smaller chunks, whilst drastically increasing the communication overhead and hindering the scalability across GPUs. This problem is bypass with data parallelism.
...
> The key idea behond **ZeRO (Zero Redundancy)** is instead of replicating, we shard the model states (optimizer states [^8], gradients, parameters) across data-parallel ranks and assemble them just-in-time with efficient collectives, _exploiting the fundamentally temporal nature of training models_. We keep only our slice/layer of parameters most of the time, and briefly all-gather when we need the rest, then discard. 

Fundamentally ZeRO exists to solve the scaling problem - models are not becoming too large to fit into a single device, and the amount of computation a processor needs to service generally scales linearly with both the number of parameters and the sequence length; the memory footprint that corresponds to a single model will include the model parameters, activations, optimizers and gradients - this is a lot. Note that the peak memory footprint typically consists of the sum of the activation memory footprint, and the steady state memory footprint.
...
[^7]: Another very useful metric is _model flops utilization (MFU)_ which is the actual FLOPs per second achieved against the peak FLOPs per second that is actually achievable on the hardware, presented as a ratio between 0 and 1.
[^8]: Optimizer states tend to dominate the memory footprint requirements. 
[^9]: Here we avoid the problem of bubbles that we run into in pipeline parallelism, and the size of the computation step is reduced (at the cost of greater inter-device communication) because we split up the tensors across each of the devices; Frequent calls of `reduceScatter` and `allGather` and `allReduce` are required. For column-wise split the gradient is split between devices and we use an `allReduce` to gather the gradient values, but with row-wise tensor-parallelism the partial gradient is on each device, so the gradient tensor can concatenate with `allGather`. 
[^10]: This is also limited by the number of devices that we can attach to a single host. 