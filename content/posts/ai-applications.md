...
> Every kernel launch is an expensive synchronization point between the CPU and GPU and forces global memory persistence of all the intermediates. By fusing all the operations within a single fused kernel loop, we minimize this unnecessary overhead, we are only able to do this kernel fusion because tiling reduces the memory footprint (more specifically it decreases the _**peak**_ tensor operation burden helf on-chip, though the total number of operations performed in a single kernel is still the same regardless of whether we tile or not) enough to fit the whole fused computation on-chip.
